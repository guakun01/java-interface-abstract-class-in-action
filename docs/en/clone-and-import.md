...
`git clone https://github.com/ByteLegendQuest/java-interface-abstract-class-in-action`

A project directory named `java-interface-abstract-class-in-action` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-interface-abstract-class-in-action` directory which is created just now.
